...
import tappidigi.composeapp.generated.resources.delete
...
import xt.qc.tappidigi.utils.removeLastChar
...
fun ChatEmojisComponent(
    ...
    Column(
        modifier = Modifier.background(Color.White).heightIn(0.dp, animatedSize).padding(8.dp)
    ) {
        if (groups == null) {
                ...
                HorizontalPager(
                    ...
                    LazyVerticalGrid(
                        ...
                        items(groups.values.elementAt(page).size) { index ->
                            ...
                            Box(
                                modifier = Modifier.size(40.dp).pointerInput(emoji) {
                                    detectTapGestures(onTap = {
                                        val newValue = contentController.value.text + emoji.character
